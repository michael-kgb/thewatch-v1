...
<section id="shopping-bag">
            <?php if (isset($customerInfo['paymentMethod'])) { ?>
                <?php
            ...
            <?php } ?>
                        <?php if (count($items) > 0) { ?>
                            ...
                            ?>
                            <?php foreach ($items as $item) { ?>
                                <?php //if(in_array($item['product_id'], $promoPermata)){ $hasPromoPermata = TRUE; } ?>
                                <?php
                                    // discount 10%
									...
									$special_promo_id = $transaction_id->special_promo_id;
									$promoInstallmentAmount = $total_special_promo;
                                ?>
                                
                                <?php $grandTotal += $item['original_product_price'] * $item['product_quantity']; ?>
                                <?php $i++; ?>
                            <?php } ?>
                        <?php } ?>
                           ...
                           <!--  IDR --> 
                            ...
                            $carrier_cost = backend\models\CarrierCost::findOne($_SESSION['customerInfo']['shippingMethod']['shipping_method']);
                            $carrier_package_detail = backend\models\CarrierPackageDetail::findOne(['carrier_package_detail_id' => $carrier_cost->carrier_package_detail_id]);
                            $district = backend\models\District::findOne(['district_id' => $carrier_cost->district_id]);
                            $state = backend\models\State::findOne(['state_id' => $district->state_id]);
                            
                            $discount = 0;
                            if($orderCartRule != NULL) {
                                $discount = $orderCartRule->value;
                            }
                                if($free_shipping==1){
									$shippingPrice = 0;
								}else{
									if($grandTotal - $discount >= 3000000) { 
											
									} elseif($grandTotal - $discount >= 1000000 && $grandTotal - $discount < 3000000) { 
											...
											if($carrier_cost->carrier_package_detail_id == 1){
												...
												$carrier_cost_flat_price = backend\models\CarrierCostFlatPrice::findOne(['province_id' => $state->province_id,'carrier_package_id' => $carrier_package_detail->carrier_package_id]);
												/*
													Free Shipping
													3 - 10 Mei
									...
									} elseif($grandTotal - $discount < 1000000){
											$carrier_cost_flat_price = backend\models\CarrierCostFlatPrice::findOne(['province_id' => $state->province_id,'carrier_package_id' => $carrier_package_detail->carrier_package_id]);
											$shippingPrice = $carrier_cost_flat_price->price * $weight;
									}else {
											$carrier_cost_flat_price = backend\models\CarrierCostFlatPrice::findOne(['province_id' => $state->province_id,'carrier_package_id' => $carrier_package_detail->carrier_package_id]);
                                ...
                                // $shippingPrice = $customerInfo['shippingMethod']['shipping_price'];
                            ?>    
            ...
            <div class="col-lg-12 col-md-12 col-sm-12 step-purchase shipping box clearleft clearright">
                ...
                <form method="post" name="ePayment" action="<?php echo \Yii::$app->params['ipay_conf']['api_url']; ?>" id="confirm-installment">
                    ...
                    $amount = (($grandTotal + $shippingPrice + $shippingInsurance) - $discount - $promoInstallmentAmount);
                    $amount00 = '00';
                    $currency = "IDR";
                    $xfield1 = '';
					...
					if($hasPromoMandiriPermata){
				// 		$promoPermataCode = '||promopermata:5||';
						$source = $merchantkey . $merchantcode . $refno . $amount . $amount00 . $currency . $xfield1 . $promoPermataCode;
					} if($hasPromoBCA){
						$source = $merchantkey . $merchantcode . $refno . $amount . $amount00 . $currency . $xfield1 . $promoBCACode; // special promo bank/payment bca instalment
					} else {
					...
					<!--<input type="hidden" name="Test" value="<?php echo $free_shipping; ?>">-->
                    <input type="hidden" name="MerchantCode" value="<?php echo $merchantcode; ?>">
                    <input type="hidden" name="PaymentId" value="1">
                    <input type="hidden" name="RefNo" value="<?php echo $_SESSION['lastOrder']['order_number']; ?>">
					...
					<?php if($hasPromoMandiriPermata){ ?>
					<input type="hidden" name="xfield1" value="<?php echo $xfield1.$promoPermataCode; ?>">
					<?php } elseif($hasPromoBCA){ // special promo bank/payment bca instalment ?>
					<input type="hidden" name="xfield1" value="<?php echo $xfield1.$promoBCACode; ?>">
					<?php } else { ?>
                    ...
                    <input type="hidden" name="ResponseURL" value="<?php echo \yii\helpers\Url::base(true);?>/tools/respon">
                    <input type="hidden" name="BackendURL" value="<?php echo \yii\helpers\Url::base(true);?>/tools/responbackend">
