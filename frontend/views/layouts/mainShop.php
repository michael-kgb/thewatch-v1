...
<html lang="<?= Yii::$app->language ?>">
    <head>
        ...
        <script type="text/javascript">
            ...
            function throw_ajax_json(url, type, data, beforeSendT, successT, errorT){
                $.ajax({
                    type: type,
                    url: url,
    ...
    <body id="page-top" class="index">	
       ...
       $bulkhead = $command->queryAll();
        ...
        $current_date = date('Y-m-d H:i:s');
            if( ($current_date > '2020-01-23 00:00:00') && ($current_date <= '2020-01-30 23:59:59') ){
                echo Yii::$app->view->renderFile('@app/views/site/chinese_new_year.php');
            }
        ?>
        <!-- Bulkhead Footer -->
        ...
        <div id="o-wrapper" class="o-wrapper">
            <main class="o-content">
                <div class="o-container">
					...
					<!--<section class="hidden-xs" style="background-repeat: repeat-x;padding: 33px 0;background-color: #f2f1f0;"></section>-->
...
// 			});
    ...
    <script>
        ...
        $('.product-care').click(function () {
            $('.product-care').fadeOut('slow');
            $('.product-care-after').slideDown('slow');
            $('.contain-care').slideDown('slow');
        });

        $('.product-care-after').click(function () {
            $('.product-care').fadeIn('fast');
            $('.product-care-after').slideUp('slow');
            $('.contain-care').slideUp('slow');
        });
...
//                $("#countdown-product-box").hide();
		...
		// $('#countdown-flash-sale').timeTo(new Date(timeflash + ' GMT+0700 (Western Indonesia Time)'));
        
        /*
		$('#countdown-flash-sale').timeTo({
			timeTo: new Date(new Date(timeflash + ' GMT+0700 (Western Indonesia Time)')),
			displayDays: 0,
			theme: "white",
		...
		$('#countdown-pre-flash-sale').timeTo({
			timeTo: new Date(new Date(pretimeflash + ' GMT+0700 (Western Indonesia Time)')),
			displayDays: 2,
			theme: "white",
			displayCaptions: true,
			fontSize: 33,
        ...
        */
        ...
        // $('#myCounter').mbComingsoon({ expiryDate: new Date(Date.parse('25 January 2019 11:00:00')), speed:100 });

        var note = $('#note'),
		ts = new Date(2019, 0, 30, 11),
		newYear = true;
        $('#countdown-pre-flash-sale').countdown({
            timestamp	: new Date(Date.parse(pretimeflash_mb)),
        ...
        $('#countdown-flash-sale').countdown({
            timestamp	: new Date(Date.parse(timeflash_mb)),
            callback	: function(days, hours, minutes, seconds){
                //
            }
        });
...
$(document).ready(function(){
    ...
    var swiper8 = new Swiper('.swiper-flash-menu-mobile', {
    			autoHeight: true,
