 ...
 */
class JewelryfeaturedbrandsController extends Controller
{
    ...
    public function actionIndex() {
        $data = \backend\models\BrandsFeaturedJewelry::find()->orderBy('brands_featured_jewelry_id DESC')->all();
        $module = \backend\models\Module::find()->where(['module_controller' => Yii::$app->controller->id])->one();
        $permissions = \backend\models\Permissions::find()->where(['group_id' => Yii::$app->session->get('userInfo')['group_id'], 'module_id' => $module->id])->one();
    ...
    public function actionCreate() {
        $model = new \backend\models\BrandsFeaturedJewelry();
        
        if ($model->load(Yii::$app->request->post())) {

            $image = UploadedFile::getInstance($model, 'brand_featured_jewelry_1');
            $image_sq = UploadedFile::getInstance($model, 'brand_featured_jewelry_2');
            

            $filename = round(microtime(true)) . '.' . $image->extension;
            $filename_sq = round(microtime(true)) . 'sq.' . $image_sq->extension;
            $model->brand_featured_jewelry_1 = $filename;
            $model->brand_featured_jewelry_2 = $filename_sq;
            
            $model->brand_status = $_POST['BrandsFeaturedJewelry']['brand_status'];
            $model->brand_link_rewrite_1 = $_POST['BrandsFeaturedJewelry']['brand_link_rewrite_1'];
            $model->brand_link_rewrite_2 = $_POST['BrandsFeaturedJewelry']['brand_link_rewrite_2'];
       //    echo $model->brand_status;die();
            
            $model->save();
            $this->upload($image, $filename);
            $this->upload($image_sq, $filename_sq);
            ...
            $log = new \backend\models\Log();
                ...
                $log->id_onChanged = $model->brands_featured_jewelry_id;
    ...
    public function actionDelete($id) {
        ...
        //$campaign = $this->findModel($id);
             ...
             $model = $this->findModel($id);
             $this->deleteFile($model->brand_featured_jewelry_1);
             $this->deleteFile($model->brand_featured_jewelry_2);
             $model->delete();
    ...
    private function upload($image, $filename) {
        ...
        $image->saveAs('../../frontend/web/img/jewelry_featured/'. $filename);
    }
    private function deleteFile($filename) {
        unlink('../../frontend/web/img/jewelry_featured/'. $filename);
    }
    ...
    protected function findModel($id) {
        if (($model = \backend\models\BrandsFeaturedJewelry::findOne($id)) !== null) {
            return $model;
        } else {
