...
class ProductsubcategoryController extends Controller
{
    ...
    public function actionIndex() {
        $data = \backend\models\ProductSubCategory::find()->orderBy('product_category_id DESC')->all();
        $module = \backend\models\Module::find()->where(['module_controller' => Yii::$app->controller->id])->one();
        $permissions = \backend\models\Permissions::find()->where(['group_id' => Yii::$app->session->get('userInfo')['group_id'], 'module_id' => $module->id])->one();
    ...
    public function actionCreate() {
        $model = new \backend\models\ProductSubCategory();
        
        if ($model->load(Yii::$app->request->post())) {

            $image = UploadedFile::getInstance($model, 'product_sub_category_image');
            $image_mobile = UploadedFile::getInstance($model, 'product_sub_category_image_mobile');
            

            $filename = round(microtime(true)) . '.' . $image->extension;
            $filename_mobile = round(microtime(true)) . 'm.' . $image_mobile->extension;
            

            $model->product_sub_category_image_mobile = $filename_mobile;
            $model->product_sub_category_image = $filename;
            
            $model->product_sub_category_sequence = count(\backend\models\ProductSubCategory::find()->all())+1;
            // $model->has_child = 0;
          ...
          //  print_r($model);die();
            ...
            $log = new \backend\models\Log();
                ...
                $log->id_onChanged = $model->product_sub_category_id;
    ...
    public function actionSequence(){
        $data = ProductSubCategory::find()->orderBy('product_sub_category_sequence ASC')->where(['product_sub_category_status' => 'active'])->all();
    ...
    public function actionReordering(){
        ...
        foreach ($items as $key => $value) {
            $key += 1;
            $homebanner = \backend\models\ProductSubCategory::findOne(['product_sub_category_id' => $value]);
            $homebanner->product_sub_category_sequence = $key;
            $homebanner->save();
        }
    }
    /**
    ...
    public function actionUpdate($id) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($image != NULL) {
                ...
                $filename = $model2->product_sub_category_image;
                ...
                $model->product_sub_category_image = $newFilename;
                ...
                $model->product_sub_category_image = $keepImages->product_sub_category_image;
            ...
            if ($image_mobile != NULL) {
                $model2 = $this->findModel($id);
                $filename_mobile = $model2->product_sub_category_image_mobile;
                ...
                $model->product_sub_category_image_mobile = $newFilename_mobile;
                ...
                $model->product_sub_category_image_mobile = $keepImages->product_sub_category_image_mobile;
    ...
    protected function findModel($id) {
        if (($model = \backend\models\ProductSubCategory::findOne($id)) !== null) {
            return $model;
        } else {
