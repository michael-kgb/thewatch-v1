...
class JournalController extends Controller {
    ...
    public function actionIndex() {
        $data = \backend\models\Journal::find()->all();
        $module = \backend\models\Module::find()->where(['module_controller' => Yii::$app->controller->id])->one();
        $permissions = \backend\models\Permissions::find()->where(['group_id' => Yii::$app->session->get('userInfo')['group_id'], 'module_id' => $module->id])->one();
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Journal'])) {
            try {
                ...
                $journal_detail->journal_id = $journal_id;
                $journal_detail->journal_detail_title = $_POST['JournalDetail']['journal_detail_title'];
				$journal_detail->journal_short_description = $_POST['JournalDetail']['journal_short_description'];
				...
				$journal_detail->journal_video_source = $_POST['JournalDetail']['journal_video_source'];
				$journal_detail->journal_video_type = $_POST['JournalDetail']['journal_video_type'];
				
				$linkRewrite = str_replace(' ', '-', $_POST['JournalDetail']['journal_detail_title']); // Replaces all spaces with hyphens.

                $journal_detail->link_rewrite = strtolower(preg_replace('/[^A-Za-z0-9\-]/', '', $linkRewrite));
                if(isset($_POST['JournalDetail']['show_product'])){
                    $journal_detail->show_product = $_POST['JournalDetail']['show_product'];
                }
                $journal_detail->product_collection_title = $_POST['JournalDetail']['product_collection_title'];
				
                $journal_detail->save();
                
                if(isset($_POST['related_product'])){
                    $related_products = $_POST['related_product'];
                    foreach ($related_products as $row) {
                      
                        $journal_product = new \backend\models\JournalDetailProduct();
                        $journal_product->journal_detail_id = $journal_detail->journal_detail_id;
                        $journal_product->product_id = $row;
                        $journal_product->save();
                ...
                $journal_image_home_cover = new \backend\models\JournalImage();
                $journal_image_home_cover->journal_id = $journal->journal_id;
                $journal_image_home_cover->orientation = 'L';
                $journal_image_home_cover->position = 0;
                $journal_image_home_cover->small_cover = null;
                $journal_image_home_cover->big_cover = 0;
                $journal_image_home_cover->home_cover = 1;
                $journal_image_home_cover->save();

                $image_home_cover = $_FILES['JournalImageHomeCover']['tmp_name'];
                $extension = explode(".", $_FILES["JournalImageHomeCover"]["name"]);
                $filename = 'home_cover_' . $journal_image_big_cover->journal_image_id . '.' . 'jpg';
                move_uploaded_file($image_home_cover, $imagePath . '/' . $filename);
                ...
                if(!empty($_FILES['JournalImageSliderImage'])){
                    for ($i = 0; $i < count($_FILES['JournalImageSliderImage']['name']); $i++) {
                        
                        if($_FILES['JournalImageSliderImage']['name'][$i] != ''){
                            $journal_image_slider = new \backend\models\JournalImageSlider();
                            $journal_image_slider->journal_id = $journal->journal_id;
                            $journal_image_slider->journal_image_slider_image = 'slide_'.$_FILES['JournalImageSliderImage']['name'][$i];
                            $journal_image_slider->journal_image_slider_active = 1;
                            $journal_image_slider->save();
    
                            $image_slider = $_FILES['JournalImageSliderImage']['tmp_name'][$i];
                          
                            $filename = $journal_image_slider->journal_image_slider_image;
                            move_uploaded_file($image_slider, $imagePath . '/' . $filename);
                        }
    ...
    public function actionUpdatecontent() {
		...
		$journalVideoSource = $_POST['journalVideoSource'];
		$journalVideoType = $_POST['journalVideoType'];
		...
		$journaldetail->journal_short_description = $shortDescription;
		...
		$journaldetail->journal_video_source = $journalVideoSource;
		$journaldetail->journal_video_type = $journalVideoType;
		
        $journaldetail->link_rewrite = $link_seo;
		$linkRewrite = str_replace(' ', '-', $title); // Replaces all spaces with hyphens.

        // $journaldetail->link_rewrite = strtolower(preg_replace('/[^A-Za-z0-9\-]/', '', $linkRewrite));
        $journaldetail->save();
    }
    ...
    public function actionUploadhomecover() {
        $journalimage = \backend\models\JournalImage::find()->where(['big_cover' => 1, 'journal_id' => $_POST['journal_id']])->one();
        $imagePath = \Yii::getAlias("@frontend" . '/web/img/journal/' . $_POST['journal_id']);
        unlink($imagePath . '/home_cover_' . $journalimage->journal_image_id . '.jpg');

        $small_cover = $_FILES['image']['tmp_name'];
        $extension = explode(".", $_FILES["image"]["name"]);
        $filename = 'home_cover_' . $journalimage->journal_image_id . '.' . 'jpg';
        move_uploaded_file($small_cover, $imagePath . '/' . $filename);

        return json_encode('<img src="../../../../frontend/web/img/journal/' . $_POST['journal_id'] . '/home_cover_' . $journalimage->journal_image_id . '.jpg" class="img-responsive"/>');
    }
    ...
    public function actionDeleteimageslider() {
        $image_id = $_POST['id'];

        $journal_image = \backend\models\JournalImageSlider::findOne($image_id);
        $journal_id = $journal_image->journal_id;
        $journal_slider = $journal_image->journal_image_slider_image;
        $journal_image->delete();

        $imagePath = \Yii::getAlias("@frontend" . '/web/img/journal/' . $journal_id);
        unlink($imagePath . '/' . $journal_slider );
    }
    ...
    public function actionUploadslider() {
        ...
        for ($i = 0; $i < $total_image; $i++) {
            $journal_image_slider = new \backend\models\JournalImageSlider();
                        $journal_image_slider->journal_id = $_POST['journal_id'];
                        $journal_image_slider->journal_image_slider_image = 'slide_'.$_FILES['image_upload' . $i]['name'];
                        $journal_image_slider->journal_image_slider_active = 1;
                        $journal_image_slider->save();
                        ...
                        $image_slider = $_FILES['image_upload' . $i]['tmp_name'];
