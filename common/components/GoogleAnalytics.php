...
class GoogleAnalytics extends Component {
    ...
    public static function getNewUsers($type) {
        $filter = $type == 'id' ? 'country == Indonesia' : 'country != Indonesia';
        $ga = new gapi('706995218108-e360a353pohtsnkgod3ur8ge1rna37vf@developer.gserviceaccount.com', 'cobalagi-5bd6223a380a.p12');
        $ga->requestReportData('79555703', null, array('newUsers'), '', $filter, date("Y-m-d"), date("Y-m-d"));
        return $ga->getNewUsers();
    }
    
    public static function getPageviews($type) {
        $filter = $type == 'id' ? 'country == Indonesia' : 'country != Indonesia';
        $ga = new gapi('706995218108-e360a353pohtsnkgod3ur8ge1rna37vf@developer.gserviceaccount.com', 'cobalagi-5bd6223a380a.p12');
        $ga->requestReportData('79555703', null, array('pageviews'), '', $filter, date("Y-m-d"), date("Y-m-d"));
        return $ga->getPageviews();
    }
    
    public static function getBounceRate($type) {
        $filter = $type == 'id' ? 'country == Indonesia' : 'country != Indonesia';
        $ga = new gapi('706995218108-e360a353pohtsnkgod3ur8ge1rna37vf@developer.gserviceaccount.com', 'cobalagi-5bd6223a380a.p12');
        $ga->requestReportData('79555703', null, array('bounceRate'), '', $filter, date("Y-m-d"), date("Y-m-d"));
        return $ga->getBounceRate();
    }
    
    public static function getSessionIsoCode($startdate, $enddate) {
        $ga = new gapi('706995218108-e360a353pohtsnkgod3ur8ge1rna37vf@developer.gserviceaccount.com', 'cobalagi-5bd6223a380a.p12');
        $myDimensions = array('countryIsoCode');
        $myMetrics = array('sessions');

        $ga->requestReportData('79555703', $myDimensions, $myMetrics, '-sessions', '', $startdate, $enddate);
